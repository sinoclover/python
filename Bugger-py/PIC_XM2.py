...
def getHeader():
    ...
    headers = {
        ...
        'Referer': 'https://nudegirls4u.com/category/ha-my-tuong/page/',
        'User-Agent': ua
    }
    return headers
...
def getListOfPic_url(listPicurl, html):
    ...
    pic_url = soup.find('div', attrs={'id': 'content'})
    all_a = pic_url.find_all('a', attrs={'class': 'tc-grid-bg-link'})
    for a in all_a:
        try:
            href = a.attrs['href']
...
def getPicInfo(listPicurl):
    ...
    for url in listPicurl:
        ...
        listPicInfo = []
        soup = bs4.BeautifulSoup(html, 'html.parser')
        all_a = soup.find_all('a', attrs={'rel': 'flickrGal0'})
        for a in all_a:
            try:
                href = a.attrs['href']
        ...
        counter += 1
        print('\r {:.2f}% of this page has been done.'.format(counter * 100 / 21), end='')
...
def main():
    for counter in range(1, 4):
        url = 'https://nudegirls4u.com/category/ha-my-tuong/page/' + str(counter)
        listPicurl = []
        html = getHTMLText(url)
