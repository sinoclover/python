...
class SkierClass(Gameing.sprite.Sprite):
    def __init__(self):
        Gameing.sprite.Sprite.__init__(self)
        self.image = Gameing.image.load("skier_down.png")
    ...
    def turn(self, direction):
        ...
        self.image = Gameing.image.load(skier_images[self.angle])
        self.rect = self.image.get_rect()
...
class ObstacleClass(Gameing.sprite.Sprite):
    def __init__(self, image_file, location, type):
        Gameing.sprite.Sprite.__init__(self)  # call Sprite initializer
        self.image_file = image_file
        self.image = Gameing.image.load(image_file)
...
def animate():
    ...
    Gameing.display.update(obstacles.draw(screen))
    screen.blit(skier.image, skier.rect)
    screen.blit(score_text, [10, 10])
    Gameing.display.flip()
...
while True:
    ...
    for event in Gameing.event.get():
        if event.type == Gameing.QUIT: sys.exit()
        if event.type == Gameing.KEYDOWN:  # check for key presses
            if event.key == Gameing.K_LEFT:  # left arrow turns left
                speed = skier.turn(-1)
            elif event.key == Gameing.K_RIGHT:  # right arrow turns right
                speed = skier.turn(1)
    skier.move(speed)
    ...
    if hit:
        if hit[0].type == "tree" and not hit[0].passed:  # crashed into tree
            points = points - 100
            skier.image = Gameing.image.load("skier_crash.png")  # crash image
            animate()
            Gameing.time.delay(1000)
            skier.image = Gameing.image.load("skier_down.png")  # resume skiing
            skier.angle = 0
            speed = [0, 6]
            hit[0].passed = True
